...
class NewsletterSubscriber(models.Model):
    """
    www 뉴스레터 구독 원장 (테이블: newsletter_subscriber, newsLetterModelPlan.md §3-1)
    """

    SIGNUP_WEB_MODAL = 'WEB_MODAL'
    SIGNUP_MEMBER_SIGNUP = 'MEMBER_SIGNUP'
    SIGNUP_SOURCE_CHOICES = [
        (SIGNUP_WEB_MODAL, '웹 모달'),
        (SIGNUP_MEMBER_SIGNUP, '회원가입'),
    ]

    STATUS_SUBSCRIBED = 'SUBSCRIBED'
    STATUS_UNSUBSCRIBED = 'UNSUBSCRIBED'
    SUBSCRIBE_STATUS_CHOICES = [
        (STATUS_SUBSCRIBED, '구독'),
        (STATUS_UNSUBSCRIBED, '구독 취소'),
    ]

    subscriber_id = models.BigAutoField(primary_key=True, verbose_name='구독자 PK')
    ...
    subscribe_status = models.CharField(
        max_length=20,
        choices=SUBSCRIBE_STATUS_CHOICES,
        default=STATUS_SUBSCRIBED,
        verbose_name='구독 상태',
    )
    agree_datetime = models.DateTimeField(null=True, blank=True, verbose_name='구독·재구독 확정 시각')
    unsubscribe_datetime = models.DateTimeField(null=True, blank=True, verbose_name='구독 취소 확정 시각')
    ip_address = models.CharField(max_length=45, null=True, blank=True, verbose_name='구독 요청 IP')
    user_agent = models.TextField(null=True, blank=True, verbose_name='구독 요청 User-Agent')
    stibee_sync_status = models.CharField(
        max_length=20,
        default='PENDING',
        verbose_name='스티비 동기화 상태',
    )
    stibee_subscriber_id = models.CharField(max_length=255, null=True, blank=True, verbose_name='스티비 구독자 식별자')
    ...
    class Meta:
        ...
        ordering = ['-create_at']
        indexes = [
            models.Index(fields=['create_at'], name='idx_newsletter_create_at'),
            models.Index(fields=['member_id'], name='idx_newsletter_member_id'),
        ]
